...
The best way to meet our requirement is to `index` the documents in advance.
...
In order to take advantages of inverted index in retrieval time, we need to build it in advance. The steps for building the inverted index are:

- Collect the documents to be indexed.
- Tokenization: turning each document to a list of tokens
...
Tokenization and normalization is beyond the scope of this article.
...
- Intersect the two postings list and return it to user
...
- Term frequency ($tf_{t,d}$): Number of times that a term occurs in a document, and the more times a term occurs in a document, the more relevant that document is.
